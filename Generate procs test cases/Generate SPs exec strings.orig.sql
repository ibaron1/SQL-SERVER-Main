...
begin
	...
	select @params# = isnull(max(rowNumber),0) from #procparam where paramname is not null
	...
	select 'GO'
	insert #procparam_parsed
	select 'declare @return int, @start datetime'
	insert #procparam_parsed
	select 'select @start = getdate()'
	insert #procparam_parsed
	select ' '

	insert #procparam_parsed
	select 'declare '+paramname+' '+datatype
	from #procparam
...
print @procname
    ...
    select ' '
    insert #procparam_parsed
    select 'select @return as return_code'
    insert #procparam_parsed
    select 'select datediff(ms, @start, getdate()) as RunTime_ms'
    insert #procparam_parsed
    select 'GO'
