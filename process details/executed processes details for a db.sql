...
--order by dbname,max_elapsed_time desc, execution_count -- by max CPU elapsed time
...
declare @boottime varchar(24)
select @boottime = crdate from master..sysdatabases where name='tempdb'

;with cte as 
(select @@servername as [Instance name], '' as dbname, '' as ObjectName,'' as ObjType, '' as total_execution_count,
 @boottime as [Since instance recycle on] 
union
select '', dbname, ObjectName, ObjType, cast(total_execution_count as varchar(max)),'' from @tmp1
)
select * from cte
order by dbname,ObjType, cast(total_execution_count as int) desc
