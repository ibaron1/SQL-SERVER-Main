...
while 1 = 1
  begin 
	...
	begin
		select @sqlToAddUQ = @sqlToAddUQ+
		   u.COLUMN_NAME+case is_descending_key
				when 0 then ' asc' else ' desc' end+
		   case when @keycnt_cur < @keycnt then ',' else ')' end 
		from INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE u
		join sys.index_columns c
		on object_id(u.TABLE_SCHEMA+'.'+u.TABLE_NAME) = c.object_id
		where CONSTRAINT_NAME = @UniqueConst and index_column_id = @keycnt_cur
