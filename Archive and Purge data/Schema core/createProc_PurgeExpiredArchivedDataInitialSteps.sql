...
****************************************************************************************************************************************/
ALTER proc [core].[PurgeExpiredArchivedDataInitialSteps]
...
begin
	...
	(DbName,
	 ...
	 select DbArchivingName,Tbl,AppDataType,workflowId,case when RetentionDaysForArchiving is not null then 'Y' else 'N' end
	 ...
	 update a
	 set PurgeExpiredDataOlderThan = dateadd(dd,-1*(c.RetentionDaysForArchiving),@DataPurgeStart),StartDate = @DataPurgeStart
	 ...
	 -- purged expired archived data 
	 set @PurgeExpiredDataOlderThan = 
	 (select top(1) convert(char(10), dateadd(dd,-1*(RetentionDays+RetentionDaysForArchiving),ctrl.DataArchivingStarted),101)
	 ...
	 delete core.loadIdsForDataPurging

	 insert core.loadIdsForDataPurging
	 select loadId
	 from tfm.FileLoad
...
begin
	 ...
	 --move data archiving from previous run into a history of archiving summary
	   ...
	   where DbName = db_name() and AppDataType = @AppDataType
		...
		set CurrentRunStarted = @StartOfCurrentRun
		,CurentRunEnded = null
		,Orig_DbSpace_Used_GB = Space_used_in_GB
