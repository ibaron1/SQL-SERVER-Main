/*****************************************************************************
Created by Eli Baron on 9/10/2024
to extract all tables, indexes, PKs
*****************************************************************************/
 --DataMart_History DataMart
DECLARE @SchemaName VARCHAR(40)= 'DataMart_History';
...
FROM 
    ...
    JOIN sys.schemas AS sc ON t.[schema_id] = sc.[schema_id] AND t.[schema_id] = SCHEMA_ID(@SchemaName)
    ...
    OUTER APPLY ( 
        SELECT 
            ...
            FORMAT(SUM([ps].[row_count]),'N0') AS row_count
        FROM
            sys.partitions AS p
