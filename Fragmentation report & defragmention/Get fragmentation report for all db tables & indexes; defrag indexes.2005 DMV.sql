...
index_level, 
page_count,
fragment_count,avg_fragment_size_in_pages,record_count,
...
and page_count >= 100 and avg_fragmentation_in_percent >= 5 
--and (index_level in (0,1) or (index_level >= 2 and page_count >= 100))
order by avg_fragmentation_in_percent desc

/* SQL TO DEFRAG  */
select distinct 'ALTER INDEX '+i.name+' ON '+object_name(indexstats.object_id)+' REBUILD WITH (SORT_IN_TEMPDB = ON, MAXDOP = 1)' 
as [Sql to defragment indexes]
from #indexstats indexstats join sys.indexes i 
on i.object_id = indexstats.object_id
and i.index_id = indexstats.index_id
and page_count >= 100 and avg_fragmentation_in_percent >= 5 
