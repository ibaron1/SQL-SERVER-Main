...
UserName1=monuser
ReportSrvr=GLSRD
PWDir="/var/tmp/p489920/mda"
PWFile=pf
RptDir="/var/tmp/p489920/mda/indexUsage"
LogDir="${RptDir}/log"

## Dataserver list 

echo GLSRD > ${RptDir}/SrvLst
...
if [[ -s "${RptDir}/${srv}.bcp" ]];then
  ...
  echo "File for $srv is empty; report failed" >> $LogName
  echo >> $LogName
fi
