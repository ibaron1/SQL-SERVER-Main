...
if exists (select '1' from sysobjects where name = 'sp_FKoperation')
  drop proc sp_FKoperation
go

create proc sp_FKoperation
...
BEGIN   
   ...
   IF @operation IN ('DROP', 'CREATE')  
   BEGIN  
	   IF @operation = 'CREATE'  GOTO Creating
       -- drop statement  
       SET @cmd = 'ALTER TABLE [' + @FKTABLE_OWNER + '].[' + @FKTABLE_NAME   
...
exec sys.sp_MS_marksystemobject sp_FKoperation
go
grant execute on sp_FKoperation to public
go
